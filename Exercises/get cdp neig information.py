...
for host in hosts: #Looping tru the hosts
    try: #first try to access the devices
        ...
        for device in log: #for each neighbor
            if "Device ID:" in device: #making sure is a neighbor
                ...
                df.to_csv(r"C:\Users\DEOLIWX2\OneDrive - Abbott\Documents\Training\Python/test6.csv", index=False, mode="a+", header=False) #writhing a row to csv
