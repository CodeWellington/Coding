...
# API GET
def api_delete(host, og_username, og_password, rest_api):
    # API information
    api_ver = 'v2'
    og_url = "https://%s" % host
    ...
    # Retrieving JSON
    try:
        r_info = requests.delete(og_url + get_info, headers=headers, verify=False)
...
def main(host_file, file_name, rest_api):
    ...
    with ThreadPoolExecutor(max_workers=40) as executor:
        future_to_device = {executor.submit(api_delete, host, og_username, og_password, rest_api): host for host in host_file}
        ...
        for future in future_to_device:
            ...
            try:
                ...
                log_file.append(f"{str(e)} for {host}: Service not found!")
...
if __name__ == "__main__":
    ...
    rest_api = input("Enter the Rest API (e.g. /lighthouse_enrollments/{id}): ")
